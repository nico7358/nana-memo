    ...
    <div className="flex flex-col h-screen max-w-md mx-auto bg-amber-50 dark:bg-slate-900 text-slate-800 dark:text-slate-200 font-sans transition-colors duration-300">
      ...
      <main className="flex-grow overflow-y-auto p-4 space-y-8">
        ...
        >
          ...
          <div>
            <h3 className="font-bold text-red-500">デバッグ用テスト</h3>
            <p className="text-sm text-slate-600 dark:text-slate-400 mt-1 mb-3">
              .mimibkファイルを選択して、コンソールの出力を確認してください。
            </p>
            <input
              type="file"
              accept=".mimibk,.db"
              ...
              onChange={async (e) => {
                ...
                alert("デバッグを開始します。コンソールを確認してください。");

                try {
                  // テスト1: ファイルは存在するか？
                  console.log("【テスト1】ファイル情報:", {
                    name: file.name,
                    size: file.size,
                    type: file.type,
                  });
                  ...
                  // テスト2: FileReaderで読めるか？
                  console.log("【テスト2】FileReaderでの読み込みを開始...");
                  ...
                  console.log(
                    `【テスト2】FileReader 成功！ ${buffer.byteLength} バイト読み込みました。`
                  );

                  // テスト3: sql.jsは初期化できるか？
                  console.log("【テスト3】sql.jsの初期化を開始...");
                  const initSqlJs = (await import("sql.js")).default;
                  const sqlWasmUrl = (
                    await import("sql.js/dist/sql-wasm.wasm?url")
                  ).default;
                  const SQL = await initSqlJs({locateFile: () => sqlWasmUrl});
                  console.log("【テスト3】sql.js 初期化 成功！");

                  // テスト4: データベースとして開けるか？
                  console.log("【テスト4】データベースのオープンを開始...");
                  const db = new SQL.Database(new Uint8Array(buffer));
                  console.log("【テスト4】データベース オープン 成功！");

                  // テスト5: テーブル情報は取得できるか？
                  console.log("【テスト5】テーブル情報の取得を開始...");
                  const tablesResult = db.exec(
                    "SELECT name FROM sqlite_master WHERE type='table';"
                  );
                  db.close();
                  console.log("【テスト5】テーブル情報:", tablesResult);

                  alert("デバッグ成功！全てのステップが完了しました。");
